...
//                    System.out.println(Arrays.toString(matrixPI) + "\n");
                    ...
                    // Optimize Case
                        ...
                        optimize(obs, vocabList, stateList, N, M, T, matrixA, matrixB, matrixPI, probBefore, output);
     ...
     */
    private static void optimize(File obsFilename, String[] vocabList, String[] stateList, int N, int M, int T, double[][] matrixA, double[][] matrixB, double[] matrixPI, double[] probBefore, File output)
    throws NumberFormatException, IOException {
        BufferedReader br = new BufferedReader(new FileReader(obsFilename));
        String sCurrentLine = br.readLine();
        int dataset_length = Integer.parseInt(sCurrentLine);
        ...
        for (int dataSetIndex = 0; dataSetIndex < dataset_length; dataSetIndex++) {
            ...
            runBaumWelchFix(obsFilename, vocabList, stateList, N, M, T, matrixA, matrixB, matrixPI, alpha, beta, numWords, obsIndex, probBefore, output);
        }
	...
	public static void runBaumWelchFix(File obsFilename, String[] vocabList, String[] stateList, int N, int M, int T, double[][] a_matrix, double[][] b_matrix,
			double[] pi_matrix, double[][] alpha, double[][] beta, int numWords, int[] obsIndex, double[] probBefore, File output) throws NumberFormatException, IOException {
		double[][][] xi;
		double[][] gamma;
...
//		printMatrix(b_matrix, "Matrix B after optimization");
		...
		double[] probAfter = recognizeAux(obsFilename, N, vocabList, a_matrix, b_matrix, pi_matrix);
		
		NumberFormat formatter = new DecimalFormat("#0.00000");
		for (int i = 0; i < probBefore.length; i++) {
			System.out.println(formatter.format(probBefore[i]) + " " + formatter.format(probAfter[i]));
		}
